...
export default function AdminLeadersPage() {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/api/admin/content/leaders/${editingId}`, payload)
      } else {
        await api.post('/api/admin/content/leaders', payload)
      }
  ...
  return (
    <div>
      <Helmet>
        <title>Lãnh đạo | Admin</title>
      </Helmet>

      <div className="d-flex align-items-end justify-content-between mb-3">
        <div>
          <div className="badge text-bg-primary">Lãnh đạo</div>
          <h1 className="h3 fw-bold mb-0">Quản lý cán bộ / lãnh đạo</h1>
          <div className="text-muted small">Tạo danh sách, upload ảnh và hiển thị lên trang Giới thiệu.</div>
        </div>
      ...
      <div className="row g-3">
        <div className="col-12 col-lg-5">
          <div className="p-4 bg-white border rounded-4">
            <div className="fw-bold mb-2">{editingId ? 'Cập nhật' : 'Thêm'} ({activeGroupLabel})</div>
            ...
            <form className="d-flex flex-column gap-2" onSubmit={save}>
                ...
                <div className="d-flex align-items-center gap-3 flex-wrap">
                  <div
                    style={{
                      ...
                      height: 96,
                      borderRadius: 16,
                      backgroundImage: `url(${form.photoUrl || '/hero/logo.png'})`,
                  ...
                  <div className="d-flex flex-column gap-2">
                    <input
                      className="form-control"
                      placeholder="Photo URL (tự điền sau khi upload)"
                      value={form.photoUrl}
              ...
              <div className="d-flex gap-2 mt-2">
                ...
                {editingId ? (
                  <button className="btn btn-outline-secondary" type="button" onClick={startCreate}>Hủy</button>
                ) : null}
              </div>
            </form>
          </div>
        ...
        <div className="col-12 col-lg-7">
          <div className="bg-white border rounded-4 overflow-hidden">
            <div className="p-3 border-bottom fw-bold">Danh sách</div>
            <div className="table-responsive">
              <table className="table mb-0 align-middle">
                ...
                <tbody>
                  {[...grouped.entries()].flatMap(([gk, arr]) => (
                    arr.map((x) => (
                      <tr key={x.id}>
                        <td style={{ width: 80 }}>
                          <div style={{ width: 56, height: 56, borderRadius: 12, backgroundImage: `url(${x.photoUrl || '/hero/logo.png'})`, backgroundSize: 'cover', backgroundPosition: 'center', border: '1px solid rgba(0,0,0,0.06)' }} />
                        </td>
                  ...
                  {items.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="text-muted p-4">Chưa có dữ liệu.</td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
